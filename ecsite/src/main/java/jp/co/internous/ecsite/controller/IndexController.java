...
import jp.co.internous.ecsite.model.dao.PurchaseRipository;
...
public class IndexController {
	...
	@Autowired
	private PurchaseRipository PurchaseRipos;
	...
	public String purchaseApi(@RequestBody CartForm f) {
		...
		f.getCartList().forEach((c) ->{
			...
			PurchaseRipos.persist(f.getUserID(), c.getId(), c.getGoodsName(), c.getCount(), total);
		}
				);
	...
	public String historyApi(@RequestBody HistoryForm form) {
		...
		List<Purchase> history = PurchaseRipos.findHistory(Long.parseLong(userId));
